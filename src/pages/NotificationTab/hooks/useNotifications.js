...
export const useNotifications = (cpf) => {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${URLs.BASIC}/api/notifications/responsavel/${cpf}`, { headers: { Authorization: authToken}});
  ...
  const deleteNotification = async (id) => {
    try {
      await axios.delete(`${URLs.BASIC}/api/notifications/${id}`, { headers: { Authorization: authToken}});
